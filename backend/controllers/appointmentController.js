...
// ✅ Fixed: Added missing `getAppointmentById` function
...
exports.createAppointment = (req, res) => {
  ...
  db.get('SELECT id FROM time_slots WHERE doctor_id = ? AND start_time = ? AND is_available = 1', 
      ...
      db.run(insertQuery, 
        ...
        function(err) {
          ...
          db.get('SELECT name FROM doctors WHERE id = ?', [doctor_id], (err, doctor) => {
            ...
            const confirmationMessage = `You have scheduled an appointment with ${doctor ? doctor.name : 'your doctor'} at ${new Date(start_time).toLocaleString()}. Please join through this link: ${zoomLink}`;
